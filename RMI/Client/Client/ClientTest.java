...
public class ClientTest {
    ...
    public static void main(String[] args) {
        ...
        int numberOfTransactions = Integer.parseInt(args[0]);
    ...
    public static void testMultiThread(int numberOfTransactions) {
            ...
            Thread thread = new Thread() {
                public void run() {
                    ...
                    try {
                        ...
                        // T1 WRITE on Flight
                        testClient.execute(Command.AddFlight,
                                commandBuilder.writeFlight(xid, itemId, numberOfItems, pricePerItem));

                        // T1 READ on Flight
                        testClient.execute(Command.QueryFlight, commandBuilder.readFlight(xid, itemId));

                        // T1 WRITE on Room
                        testClient.execute(Command.AddRooms,
                                commandBuilder.writeRoom(xid, itemId, numberOfItems, pricePerItem));

                        // T1 READ on Room
                        testClient.execute(Command.QueryRooms, commandBuilder.readRooms(xid, itemId));

                        // T1 WRITE on Car
                        testClient.execute(Command.AddCars,
                                commandBuilder.writeCar(xid, itemId, numberOfItems, pricePerItem));

                        // T1 READ on Car
                        testClient.execute(Command.QueryCars, commandBuilder.readCars(xid, itemId));
