...
import Server.Middleware.Transaction;
				...
				synchronized (this.lockTable) {
					...
					if (!bConflict) {
						...
						if (bConvert.get(0) == true) {
							...
							// TransactionLockObject and DataLockObject for the READ lock in lockTable with lockTable.get method.
							// Remove them from lockTable then add them back with WRITE lock.
							TransactionLockObject xReadLockObject = (TransactionLockObject) this.lockTable.get(new TransactionLockObject(xid, data, LockType.LOCK_READ));
							DataLockObject dataReadLockObject = (DataLockObject) this.lockTable.get(new DataLockObject(xid, data, LockType.LOCK_READ));

							this.lockTable.remove(xReadLockObject);
							this.lockTable.remove(dataReadLockObject);
			...
			{    
				...
				else if (dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
				{
					...
					if(l_dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE){
						...
						Trace.info(dataLockObject.getXId() + " already has READ lock but converting to WRITE lock");
						bitset.set(0);
					}
				}
			} 
