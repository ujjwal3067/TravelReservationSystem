...
public class ResourceManager implements IResourceManager {
	...
	public ResourceManager(String p_name) {
		m_name = p_name;
		transactionManager = new TransactionManager(); // Initalizes a local transaction manager.
	}

	public void addTransaction(int xid) throws RemoteException {
		System.out.println("attaching transaction: " + xid);
		if (!this.transactionManager.isActive(xid)) { // if not active already
			Transaction transaction = new Transaction(xid);
			this.transactionManager.addTransaction(xid, transaction);
		}
	...
	protected RMItem readData(int xid, String key) throws InvalidTransactionException {
		...
		if (!this.transactionManager.isActive(xid)) { // transaction is not active
			throw new InvalidTransactionException(xid);
		} else {
			Transaction transaction = this.transactionManager.getTransaction(xid);
			if (!transaction.hasKey(key)) { // if transaction doesn't have local copy of this object
				synchronized (this.m_data) {
					RMItem item = this.m_data.get(key);
					if (item != null) {
						transaction.WRITE(xid, key, (RMItem) item.clone()); // add the data object copy
					} else {
						transaction.WRITE(xid, key, null); // if object not present add null :: maintains a mirror copy
						// this ways
					}
				}

			}
			return transaction.READ(xid, key);
		}
	}

	public int Start() throws RemoteException {
		throw new RemoteException("Start() is not allowed to be called on servers.");
	}

	// Writes a data item
	protected void writeData(int xid, String key, RMItem value) throws InvalidTransactionException {
		// check if transaction is valid
		if (!this.transactionManager.isActive(xid)) { // xid is not active
			throw new InvalidTransactionException(xid);
		} else {
			// we don't have to writeData directly we need to read the hashmap
			// and copy that into the transaction itself as a copy
			readData(xid, key);
			Transaction transaction = this.transactionManager.getTransaction(xid);
			transaction.WRITE(xid, key, value); // write on the local copy of the transaction
		}
	}
	...
	protected void removeData(int xid, String key) throws InvalidTransactionException {
		if (!this.transactionManager.isActive(xid)) {
			...
			readData(xid, key);
			Transaction transaction = this.transactionManager.getTransaction(xid);
			transaction.WRITE(xid, key, null); // removing data from transaction local copy
		}
	}

	/**
	 * Commits the transaction this methods access the transaction associate with
	 * xid and takes the m_data copy from transaction and commits it in it's own
	 * version of m_data ( data hashmap)
	 * 
	 * @param xid
	...
	public boolean commit(int xid) throws RemoteException, TransactionAbortedException, InvalidTransactionException {
		...
		// System.out.println(color.RED + "Committing Transaction <xid> : " + xid);
		// check if the transaction is in active transaction list or not
		if (!transactionManager.isActive(xid)) { // transaction is not active
			throw new InvalidTransactionException(xid);
		} else {
			Transaction transaction = transactionManager.getTransaction(xid);
			RMHashMap copyData = transaction.getDataCOPY(); // get the local copy of objects from the transaction
			synchronized (this.m_data) { // copy/commit all the local object to RM database
				for (String key : copyData.keySet()) {
					this.m_data.put(key, copyData.get(key));
				}
			}
			transactionManager.addTransaction(xid, null);
			return true;
		}
	}

	public void abort(int xid) throws RemoteException, InvalidTransactionException {
		// System.out.println(color.BG_BLUE + " Abort Transaction");
		System.out.println("Abort Transaction");
		// check if transaction is active or not
		if (!transactionManager.isActive(xid)) {
			throw new InvalidTransactionException(xid);
		} else {
			this.transactionManager.addTransaction(xid, null); // transaction aborted :: i.e. null entry in
			// ACTIVETransaction list
		}
	}

	// Deletes the car item
	...
	public String getName() throws RemoteException, TransactionAbortedException, InvalidTransactionException {
		return m_name;
	}
}
