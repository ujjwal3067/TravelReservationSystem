...
public class Customer extends RMItem {
	...
	public String getDetails() {
		String details = "[ Customer Summary ]" + "\n" + "----Customer " + this.m_ID + "------" + "\n";
		for (String map_key : this.m_reservations.keySet()) { // retrieve the keyset of the m_reservation
			ReservedItem item = (ReservedItem) this.m_reservations.get(map_key);
			details += item.getCount() + " " + item.getReservableItemKey() + "TOTAL AMOUNT :: $"
	...
	public void reserve(String key, String location, int price) {
		...
		if (reservedItem == null) {
			// Customer doesn't already have a reservation for this resource, so create a
			// new one now
